...
#
# create a function that generates returns malicious user id among the users randomly
def getMaliciousUser(num_users):
    return random.randint(0, num_users - 1)

def gaussian_attack(weights, mean=0, stddev=0.1):
    return [weight + np.random.normal(mean, stddev, weight.shape) for weight in weights]

def label_flipping_attack(labels):
...
for round_no in range(1, num_rounds + 1):
    ...
    for user_id in range(num_users):
        ...
        if user_id == malicious_user_id:
            # Choose an attack type: Gaussian or Label Flipping
            attack_type = random.choice(['gaussian', 'label_flipping'])
            print(f"User {user_id} is malicious and will poison the model with {attack_type} attack.")
            
            if attack_type == 'gaussian':
                poisoned_weights = gaussian_attack(local_model_user.get_weights())
                ...
                y_train_user_flipped = label_flipping_attack(y_train_user)
                train_loss, train_accuracy = train_model(local_model_user, X_train_user, y_train_user_flipped, X_test, y_test, epochs=epoch)
