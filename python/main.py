...
# Function to save plots for each user
def save_plots_for_each_user(user_losses_training, user_accuracies_testing):
    for user_id in user_losses_training.keys():
        # Plot training loss and accuracy
        plt.figure(figsize=(10, 5))
        plt.subplot(1, 2, 1)
        plt.plot(user_losses_training[user_id][::2], label='Training Loss')  # Assuming even indices are losses
        plt.title(f'User {user_id} Training Loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()

        plt.subplot(1, 2, 2)
        plt.plot(user_losses_training[user_id][1::2], label='Training Accuracy')  # Assuming odd indices are accuracies
        plt.title(f'User {user_id} Training Accuracy')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        plt.legend()

        plt.tight_layout()
        plt.savefig(f'user_{user_id}_training.png')
        plt.close()

        # Plot testing accuracy
        plt.figure()
        plt.plot(user_accuracies_testing[user_id][1::2], label='Testing Accuracy')  # Assuming odd indices are accuracies
...
save_plots_for_each_user(user_losses_training, user_accuracies_testing)